...
const [desktopStart0, desktopEnd0] =
  MILESTONES_YEAR_RANGES_BY_BREAKPOINT.desktop[0];
const firstRangeItems = milestonesData.filter(
...
describe('MilestonesPage', () => {
  describe('타이틀', () => {
    ...
    it('pageIndex=0, side=right(dataIndex=1)이면 타이틀이 없다', () => {
      render(
        <MilestonesPage side='right' pageIndex={0} breakpoint='desktop' />,
      );
      expect(
  ...
  describe('데이터 렌더링', () => {
    ...
    it('유효하지 않은 pageIndex에서도 오류 없이 빈 컨테이너를 렌더링한다', () => {
      expect(() =>
        render(
          <MilestonesPage side='left' pageIndex={99} breakpoint='desktop' />,
        ),
      ).not.toThrow();
    });
    ...
    it('tablet breakpoint에서도 오류 없이 렌더링된다', () => {
      expect(() =>
        render(
          <MilestonesPage side='left' pageIndex={0} breakpoint='tablet' />,
        ),
      ).not.toThrow();
    });
  });
